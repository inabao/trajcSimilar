...
subResult pssDTW(path p1, path p2) {
    ...
    double distanceTmp[p1.size()+1];
    distanceTmp[0] = 0;
    for (int i = 1; i < p1.size() + 1; ++i) {
        distanceTmp[i] = distanceTmp[i-1] + pointDistance(p1[i-1], nullPoint);
    }
    for (int j = 1; j < p2.size() + 1; ++j) {
        if (j == start + 1) distanceMatrixReverseTmp = distanceTmp;
        distanceMatrixReverse[0] = distanceMatrixReverseTmp[0] + pointDistance(p2[j - 1], nullPoint);
        for (int i = 1; i < p1.size() + 1; ++i) {
            distanceMatrixReverse[i] = min(distanceMatrixReverseTmp[i-1] + pointDistance(p1[i-1], p2[j-1]),
                                                      min(distanceMatrixReverseTmp[i] + pointDistance(nullPoint, p2[j-1]),
                                                          distanceMatrixReverse[i-1] + pointDistance(nullPoint, p1[i-1])));
        }
        double theta1 = distanceMatrixReverse[p1.size()];
        double theta2 = distanceMatrixTmp[p2.size() - j + 1];
        if (min(theta1,theta2) < bestScore) {
